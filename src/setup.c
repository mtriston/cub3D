...
static void	init_img(t_vars *vars, t_img *img, t_screen *screen)
{
	screen->win = \
			mlx_new_window(screen->mlx, screen->width, screen->height, "cub3D");
	if (screen->win == NULL)
		ft_exit("mlx_new_window failed", vars);
	img->img = mlx_new_image(screen->mlx, screen->width, screen->height);
	if (img->img == NULL)
		ft_exit("mlx_new_image failed", vars);
	img->addr = mlx_get_data_addr(img->img, &img->bits_per_pixel, \
									&img->line_length, &img->endian);
	if (img->addr == NULL)
		ft_exit("mlx_get_data_addr failed", vars);
}
...
void		setup(t_vars *vars, char *path)
{
	...
	if (vars->screen.mlx == NULL)
		ft_exit("MLH initialization failed", vars);
	prepare_structs(vars);
	parser(path, vars);
	init_img(vars, &vars->img, &vars->screen);
}
